"""National Hurricane Center Atlantic wind-field map and GIF tools."""
...
STORM_LAYERS = {'AT1': {'points': 6, 'rings': 16}, 'AT2': {'points': 32, 'rings': 42}, 'AT3': {'points': 58, 'rings': 68}, 'AT4': {'points': 84, 'rings': 94}, 'AT5': {'points': 110, 'rings': 120}}


def check_active_storms():
    """Check which Atlantic storm layers currently have usable data."""
    print('🔍 Checking active storms across all layers...')
    active_storms = {}
...
def calculate_gif_map_extent(gdf_points):
    ...
    if traj_width < 5.0:
        margin_x = max(9.0, traj_width * 1.0)
        margin_y = max(5.5, traj_height * 1.2)
        print('🔍 GIF - Trayectoria LOCAL detectada')
    elif traj_width < 15.0:
        margin_x = max(9.0, traj_width * 0.7)
        margin_y = max(6, traj_height * 0.9)
        print('🌊 GIF - Trayectoria REGIONAL detectada')
    else:
        margin_x = max(9.0, traj_width * 0.4)
        margin_y = max(5.5, traj_height * 0.5)
        print('🌐 GIF - Trayectoria TRANS-ATLÁNTICA detectada')
...
def draw_wind_ring(ax, poly, color, zorder_base=15):
    """Draw a wind ring while preserving the color and 3D styling."""
...
def extract_date_time(point):
    """
    Extract and format the forecast point date.

    Input: '2025-10-27 2:00 PM Mon EDT'
    Output: 'Monday October 27 - 14:00 EDT'
    """
    try:
        months = {'01': 'January', '02': 'February', '03': 'March', '04': 'April', '05': 'May', '06': 'June', '07': 'July', '08': 'August', '09': 'September', '10': 'October', '11': 'November', '12': 'December'}
        days = {'Mon': 'Monday', 'Tue': 'Tuesday', 'Wed': 'Wednesday', 'Thu': 'Thursday', 'Fri': 'Friday', 'Sat': 'Saturday', 'Sun': 'Sunday'}
        if 'fldatelbl' not in point or not point['fldatelbl']:
            return 'Date unavailable'
        fldatelbl = point['fldatelbl']
        parts = fldatelbl.split()
        ...
        day_name = days.get(day_eng, day_eng)
        hour_parts = hour_time.split(':')
        ...
        if am_pm == 'PM' and hour != 12:
            ...
            hour = 0
        result = f'{day_name} {month_name} {int(day_num)} - {hour:02d}:{minute} {timezone}'
        return result
    except Exception as e:
        print(f'⚠️  Error extracting date: {e}')
        if 'fldatelbl' in point and point['fldatelbl']:
            return point['fldatelbl']
...
def main():
    """Interactive entry point for Atlantic wind-field maps and GIFs."""
    print('🌀 MULTI-STORM WIND FIELD GENERATOR - ATLANTIC')
    print('=' * 70)
    try:
