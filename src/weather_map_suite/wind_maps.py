"""Regional wind map renderers extracted from the source notebook."""
...
def plot_iberia_wind(ds_wind, admin_boundaries=None):
    """
    Plot wind magnitude with direction vectors for Spain and Portugal (Iberian Peninsula)

    Parameters:
    -----------
    ds_wind : xarray.Dataset
        Dataset with variables 'u10', 'v10', and 'wind_speed' (wind speed in m/s)
    ...
    u10_da = ds_wind['u10']
    v10_da = ds_wind['v10']
    wind_speed_da = ds_wind['wind_speed']
    u10_corrected = u10_da.assign_coords(longitude=(u10_da.longitude + 180) % 360 - 180).sortby('longitude')
    v10_corrected = v10_da.assign_coords(longitude=(v10_da.longitude + 180) % 360 - 180).sortby('longitude')
    wind_speed_corrected = wind_speed_da.assign_coords(longitude=(wind_speed_da.longitude + 180) % 360 - 180).sortby('longitude')
    u10_area = u10_corrected.sel(latitude=slice(lat_max, lat_min), longitude=slice(lon_min_deg, lon_max_deg))
    v10_area = v10_corrected.sel(latitude=slice(lat_max, lat_min), longitude=slice(lon_min_deg, lon_max_deg))
    wind_area = wind_speed_corrected.sel(latitude=slice(lat_max, lat_min), longitude=slice(lon_min_deg, lon_max_deg))
    lat = wind_area.latitude.values
    lon = wind_area.longitude.values
    if u10_area.values.ndim == 3:
        u_vals = u10_area.values[0, :, :]
        v_vals = v10_area.values[0, :, :]
        wind_vals = wind_area.values[0, :, :]
    else:
        u_vals = u10_area.values
        v_vals = v10_area.values
        wind_vals = wind_area.values
    wind_vals_kmh = wind_vals * 3.6 + 10
    ...
    wind_vals_orig = wind_vals_kmh.ravel()
    wind_fine = griddata(points_orig, wind_vals_orig, (lon2d_fine, lat2d_fine), method='linear')
    wind_fine_display = wind_fine
    ...
    wind_colors = ['#e0f7fa', '#80deea', '#26c6da', '#00acc1', '#00838f', '#4db6ac', '#ffca28', '#ff7043', '#f44336', '#8e24aa']
    cmap_wind = ListedColormap(wind_colors)
    cmap_truncated = cmap_wind
    vmin = np.nanmin(wind_fine_display)
    vmax = np.nanmax(wind_fine_display)
    levels = np.linspace(vmin, vmax, 15)
    cf = ax.contourf(lon2d_fine, lat2d_fine, wind_fine_display, levels=levels, cmap=cmap_truncated, extend='both', transform=ccrs.PlateCarree(), alpha=0.8)
    step = 4
    wind_magnitude = np.sqrt(u_vals ** 2 + v_vals ** 2) * 3.6
    u_sub = u_vals[::step, ::step]
    v_sub = v_vals[::step, ::step]
    wind_mag_sub = wind_magnitude[::step, ::step]
    lon_sub = lon2d[::step, ::step]
    lat_sub = lat2d[::step, ::step]
    wind_scale = np.log1p(wind_mag_sub)
    wind_scale = wind_scale / np.nanmax(wind_scale)
    u_scaled = u_sub * (0.3 + 0.7 * wind_scale)
    v_scaled = v_sub * (0.3 + 0.7 * wind_scale)
    quiver = ax.quiver(lon_sub, lat_sub, u_scaled, v_scaled, transform=ccrs.PlateCarree(), color='black', alpha=0.75, width=0.0025, scale=70, scale_units='inches', headwidth=3.5, headlength=4, headaxislength=3.5, minshaft=2, minlength=0.1, zorder=10)
    ...
    cbar.set_label('Maximum Wind Gusts (km/h)', fontsize=10, weight='bold')
    ...
    wind_values = wind_fine_display.ravel()
    for nombre, datos in major_cities.items():
        ...
        if not np.isnan(wind_speed):
            ...
            ax.text(lon_ci, lat_ci + 0.07, f'{wind_speed:.0f} ', fontsize=11.5, color='white', weight='bold', ha='center', va='bottom', transform=ccrs.PlateCarree(), zorder=15, bbox=dict(boxstyle='round,pad=0.2', facecolor='#1a5490', edgecolor='white', linewidth=1.3, alpha=1), path_effects=[PathEffects.Normal()])
    ...
    output_file = f'output/maps/iberia/iberia_map_wind.png'
...
def plot_canary_islands_wind(ds_wind, admin_boundaries=None):
    """
    Plot wind magnitude with direction vectors for the Canary Islands
    ...
    lat_min = 26.5
    lat_max = 30.5
    lon_orig_u = ds_wind['u10'].longitude.values
    lon_corrected_u = np.where(lon_orig_u > 180, lon_orig_u - 360, lon_orig_u)
    u10_corrected = ds_wind['u10'].assign_coords(longitude=lon_corrected_u)
    v10_corrected = ds_wind['v10'].assign_coords(longitude=lon_corrected_u)
    wind_speed_corrected = ds_wind['wind_speed'].assign_coords(longitude=lon_corrected_u)
    u10_area = u10_corrected.sel(latitude=slice(lat_max, lat_min), longitude=slice(lon_min_deg, lon_max_deg))
    v10_area = v10_corrected.sel(latitude=slice(lat_max, lat_min), longitude=slice(lon_min_deg, lon_max_deg))
    wind_area = wind_speed_corrected.sel(latitude=slice(lat_max, lat_min), longitude=slice(lon_min_deg, lon_max_deg))
    lat = wind_area.latitude.values
    ...
    u_scaled = u_sub * (0.3 + 0.7 * wind_scale)
    v_scaled = v_sub * (0.3 + 0.7 * wind_scale)
    quiver = ax.quiver(lon_sub, lat_sub, u_scaled, v_scaled, transform=ccrs.PlateCarree(), color='black', alpha=0.75, width=0.0025, scale=60, scale_units='inches', headwidth=3.5, headlength=4, headaxislength=3.5, minshaft=2, minlength=0.1, zorder=10)
    ...
    cbar.set_label('Maximum Wind Gusts (km/h)', fontsize=11, weight='bold')
    ...
    for nombre, datos in major_cities.items():
        ...
        wind_speed = wind_values[idx]
        ax.plot(lon_ci, lat_ci, 'o', color='red', markersize=4, markeredgecolor='white', markeredgewidth=1, transform=ccrs.PlateCarree(), zorder=12)
        ax.text(lon_ci, lat_ci + 0.04, f'{wind_speed:.0f} ', fontsize=11.5, color='white', weight='bold', ha='center', va='bottom', transform=ccrs.PlateCarree(), zorder=15, bbox=dict(boxstyle='round,pad=0.2', facecolor='#1a5490', edgecolor='white', linewidth=1.3, alpha=1), path_effects=[PathEffects.Normal()])
    ...
    output_file = f'output/maps/canary_islands/canary_islands_map_wind.png'
    plt.savefig(output_file, dpi=800, bbox_inches='tight')
    print(f'✅ Map saved: {output_file}')
    plt.show()
    return (fig, ax)
...
def plot_central_america_wind(ds_wind, admin_boundaries=None):
    """
    Plot wind magnitude with direction vectors for Central America
    ...
    v10_corrected = ds_wind['v10'].assign_coords(longitude=lon_corrected_u)
    wind_speed_corrected = ds_wind['wind_speed'].assign_coords(longitude=lon_corrected_u)
    u10_area = u10_corrected.sel(latitude=slice(lat_max + 1, lat_min), longitude=slice(lon_min_deg, lon_max_deg))
    v10_area = v10_corrected.sel(latitude=slice(lat_max + 1, lat_min), longitude=slice(lon_min_deg, lon_max_deg))
    wind_area = wind_speed_corrected.sel(latitude=slice(lat_max + 1, lat_min), longitude=slice(lon_min_deg, lon_max_deg))
    lat = wind_area.latitude.values
    lon = wind_area.longitude.values
    if u10_area.values.ndim == 3:
        u_vals = u10_area.values[0, :, :]
    ...
    cbar = plt.colorbar(cf, cax=cax, orientation='horizontal')
    cbar.set_label('Wind speed (km/h)', fontsize=11, weight='bold')
    cbar.ax.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'{int(x)}'))
    cbar.ax.tick_params(labelsize=8)
    ...
    for nombre, datos in major_cities.items():
        ...
        wind_speed = wind_values[idx]
        ax.plot(lon_ci, lat_ci, 'o', color='red', markersize=4, markeredgecolor='white', markeredgewidth=1, transform=ccrs.PlateCarree(), zorder=12)
        ax.text(lon_ci, lat_ci + 0.08, f'{wind_speed:.0f} ', fontsize=7.9, color='white', weight='bold', ha='center', va='bottom', transform=ccrs.PlateCarree(), zorder=15, bbox=dict(boxstyle='round,pad=0.2', facecolor='#1a5490', edgecolor='white', linewidth=1.3, alpha=1), path_effects=[PathEffects.Normal()])
        ax.text(lon_ci, lat_ci - 0.08, nombre, fontsize=7.4, color='#2C3E50', weight='bold', ha='center', va='top', zorder=15, transform=ccrs.PlateCarree(), path_effects=[PathEffects.withStroke(linewidth=2, foreground='white'), PathEffects.SimpleLineShadow(offset=(1, -1), alpha=0.3), PathEffects.Normal()])
    try:
    ...
    output_file = f'output/maps/central_america/central_america_map_wind.png'
    plt.savefig(output_file, dpi=800, bbox_inches='tight')
    print(f'✅ Map saved: {output_file}')
    plt.show()
    return (fig, ax)
...
def plot_hispaniola_wind(ds_wind, admin_boundaries=None):
    """
    Plot wind magnitude with direction vectors for Hispaniola (Haiti and the Dominican Republic)
    ...
    for nombre, datos in major_cities.items():
        ...
        wind_speed = wind_values[idx]
        ax.plot(lon_ci, lat_ci, 'o', color='red', markersize=4, markeredgecolor='white', markeredgewidth=1, transform=ccrs.PlateCarree(), zorder=12)
        ax.text(lon_ci, lat_ci + 0.08, f'{wind_speed:.0f} km/h', fontsize=7.9, color='white', weight='bold', ha='center', va='bottom', transform=ccrs.PlateCarree(), zorder=15, bbox=dict(boxstyle='round,pad=0.2', facecolor='#1a5490', edgecolor='white', linewidth=1.3, alpha=1), path_effects=[PathEffects.Normal()])
        ax.text(lon_ci, lat_ci - 0.08, nombre, fontsize=7.4, color='#2C3E50', weight='bold', ha='center', va='top', zorder=15, transform=ccrs.PlateCarree(), path_effects=[PathEffects.withStroke(linewidth=2, foreground='white'), PathEffects.SimpleLineShadow(offset=(1, -1), alpha=0.3), PathEffects.Normal()])
    try:
        logo_img = mpimg.imread(LOGO_PATH)
    ...
    output_file = f'output/maps/hispaniola/hispaniola_map_wind.png'
    plt.savefig(output_file, dpi=800, bbox_inches='tight')
    print(f'Map saved: {output_file}')
    plt.show()
    return (fig, ax)
...
def plot_lesser_antilles_wind(ds_wind, admin_boundaries=None):
    """
    Plot wind magnitude with direction vectors for the Lesser Antilles
    ...
    output_file = f'output/maps/lesser_antilles/lesser_antilles_map_wind.png'
    plt.savefig(output_file, dpi=800, bbox_inches='tight')
    print(f'✅ Map saved: {output_file}')
    plt.show()
    return (fig, ax)
...
def plot_puerto_rico_wind(ds_wind, admin_boundaries=None, puerto_rico_counties=None):
    """
    Plot wind magnitude with direction vectors para Puerto Rico
    ...
    v10_corrected = ds_wind['v10'].assign_coords(longitude=lon_corrected_u)
    wind_speed_corrected = ds_wind['wind_speed'].assign_coords(longitude=lon_corrected_u)
    u10_area = u10_corrected.sel(latitude=slice(lat_max, lat_min - 1), longitude=slice(lon_min_deg, lon_max_deg))
    v10_area = v10_corrected.sel(latitude=slice(lat_max, lat_min - 1), longitude=slice(lon_min_deg, lon_max_deg))
    wind_area = wind_speed_corrected.sel(latitude=slice(lat_max, lat_min - 1), longitude=slice(lon_min_deg, lon_max_deg))
    lat = wind_area.latitude.values
    lon = wind_area.longitude.values
    if u10_area.values.ndim == 3:
        u_vals = u10_area.values[0, :, :]
        ...
        v_vals = v10_area.values
        wind_vals = wind_area.values
    wind_vals_mph = wind_vals * 2.23694 + 5
    lon2d, lat2d = np.meshgrid(lon, lat)
    lon_fine = np.linspace(lon.min(), lon.max(), 300)
    lat_fine = np.linspace(lat.min(), lat.max(), 300)
    lon2d_fine, lat2d_fine = np.meshgrid(lon_fine, lat_fine)
    points_orig = np.column_stack((lon2d.ravel(), lat2d.ravel()))
    wind_vals_orig = wind_vals_mph.ravel()
    wind_fine = griddata(points_orig, wind_vals_orig, (lon2d_fine, lat2d_fine), method='linear')
    wind_fine_smooth = gaussian_filter(wind_fine, sigma=1)
    wind_fine_display = wind_fine_smooth
    fig = plt.figure(figsize=(16, 8))
    ax = plt.axes(projection=ccrs.PlateCarree())
    ax.set_extent([lon_min_deg, lon_max_deg, lat_min, lat_max], crs=ccrs.PlateCarree())
    ax.add_feature(cfeature.LAND, facecolor='lightgray')
    ...
    cf = ax.contourf(lon2d_fine, lat2d_fine, wind_fine_display, levels=levels, cmap=cmap_truncated, extend='both', transform=ccrs.PlateCarree(), alpha=0.8)
    step = 1
    wind_magnitude = np.sqrt(u_vals ** 2 + v_vals ** 2) * 2.23694
    u_sub = u_vals[::step, ::step]
    v_sub = v_vals[::step, ::step]
    wind_mag_sub = wind_magnitude[::step, ::step]
    lon_sub = lon2d[::step, ::step]
    ...
    u_scaled = u_sub * (0.3 + 0.7 * wind_scale)
    v_scaled = v_sub * (0.3 + 0.7 * wind_scale)
    quiver = ax.quiver(lon_sub, lat_sub, u_scaled, v_scaled, transform=ccrs.PlateCarree(), color='black', alpha=0.75, width=0.0025, scale=50, scale_units='inches', headwidth=3.5, headlength=4, headaxislength=3.5, minshaft=2, minlength=0.1, zorder=10)
    cax = inset_axes(ax, width='42%', height='5%', loc='lower left', bbox_to_anchor=(-0.015, 0.088, 0.9, 0.9), bbox_transform=ax.transAxes, borderpad=7)
    cbar = plt.colorbar(cf, cax=cax, orientation='horizontal')
    cbar.set_label('Wind speed (mph)', fontsize=11, weight='bold')
    cbar.ax.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'{int(x)}'))
    cbar.ax.tick_params(labelsize=8)
    ...
    for nombre, datos in major_cities.items():
        ...
        wind_speed = wind_values[idx]
        ax.plot(lon_ci, lat_ci, 'o', color='red', markersize=4, markeredgecolor='white', markeredgewidth=1, transform=ccrs.PlateCarree(), zorder=12)
        ax.text(lon_ci, lat_ci + 0.02, f'{wind_speed:.0f} ', fontsize=7.9, color='white', weight='bold', ha='center', va='bottom', transform=ccrs.PlateCarree(), zorder=15, bbox=dict(boxstyle='round,pad=0.2', facecolor='#1a5490', edgecolor='white', linewidth=1.3, alpha=1), path_effects=[PathEffects.Normal()])
        ax.text(lon_ci, lat_ci - 0.02, nombre, fontsize=7.4, color='#2C3E50', weight='bold', ha='center', va='top', zorder=15, transform=ccrs.PlateCarree(), path_effects=[PathEffects.withStroke(linewidth=2, foreground='white'), PathEffects.SimpleLineShadow(offset=(1, -1), alpha=0.3), PathEffects.Normal()])
    try:
        logo_img = mpimg.imread(LOGO_PATH)
    ...
    output_file = f'output/maps/puerto_rico/puerto_rico_map_wind.png'
    plt.savefig(output_file, dpi=800, bbox_inches='tight')
    print(f'Map saved: {output_file}')
    plt.show()
    return (fig, ax)
...
def plot_colombia_venezuela_wind(ds_wind, admin_boundaries=None):
    """
    Plot wind magnitude with direction vectors para Colombia y Venezuela
    ...
    cax = inset_axes(ax, width='2.5%', height='45%', loc='lower left', bbox_to_anchor=(0.02, 0.15, 1, 1), bbox_transform=ax.transAxes, borderpad=0)
    cbar = plt.colorbar(cf, cax=cax, orientation='vertical')
    cbar.set_label('Wind speed (km/h)', fontsize=10, weight='bold')
    cbar.ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'{int(x)}'))
    cbar.ax.tick_params(labelsize=8)
    ...
    output_file = f'output/maps/colombia_venezuela/colombia_venezuela_map_wind.png'
    plt.savefig(output_file, dpi=800, bbox_inches='tight')
    print(f'✅ Map saved: {output_file}')
    plt.show()
    return (fig, ax)
...
def plot_united_states_wind(ds_wind, admin_boundaries=None):
    """
    Plot wind magnitude with direction vectors para Estados Unidos
    ...
    if u10_area.values.ndim == 3:
        ...
        v_vals = v10_area.values
        wind_vals = wind_area.values
    wind_vals_mph = wind_vals * 2.23694
    lon2d, lat2d = np.meshgrid(lon, lat)
    lon_fine = np.linspace(lon.min(), lon.max(), 400)
    lat_fine = np.linspace(lat.min(), lat.max(), 400)
    lon2d_fine, lat2d_fine = np.meshgrid(lon_fine, lat_fine)
    ...
    cbar = plt.colorbar(cf, cax=cax, orientation='horizontal')
    cbar.set_label('Rachas maximum de wind (mph)', fontsize=11, weight='bold')
    cbar.ax.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'{int(x)}'))
    cbar.ax.tick_params(labelsize=8)
    ...
    for nombre, datos in major_cities.items():
        ...
        wind_speed = wind_values[idx]
        ax.plot(lon_ci, lat_ci, 'o', color='red', markersize=4, markeredgecolor='white', markeredgewidth=1, transform=ccrs.PlateCarree(), zorder=12)
        ax.text(lon_ci, lat_ci + 0.14, f'{wind_speed:.0f} ', fontsize=11.5, color='white', weight='bold', ha='center', va='bottom', transform=ccrs.PlateCarree(), zorder=15, bbox=dict(boxstyle='round,pad=0.2', facecolor='#1a5490', edgecolor='white', linewidth=1.3, alpha=1), path_effects=[PathEffects.Normal()])
    ...
    output_file = f'output/maps/united_states/united_states_map_wind.png'
    plt.savefig(output_file, dpi=800, bbox_inches='tight')
    print(f'✅ Map saved: {output_file}')
    plt.show()
    return (fig, ax)
...
def plot_texas_wind(ds_wind, admin_boundaries=None, texas_counties=None, show_logo=True):
    """
    Plot wind magnitude with direction vectors para Texas en MPH
    ...
    if u10_area.values.ndim == 3:
        ...
        wind_vals = wind_area.values
    wind_vals_mph = wind_vals * 2.23694
    print(f'Wind speed: Min={wind_vals_mph.min():.1f}, Max={wind_vals_mph.max():.1f}, Mean={wind_vals_mph.mean():.1f} mph')
    lon2d, lat2d = np.meshgrid(lon, lat)
    ...
    for nombre, datos in major_cities.items():
        ...
        ax.plot(lon_ci, lat_ci, 'o', color='red', markersize=4, markeredgecolor='white', markeredgewidth=1, transform=ccrs.PlateCarree(), zorder=12)
        ax.text(lon_ci, lat_ci + 0.14, f'{wind_speed:.0f}', fontsize=11.5, color='white', weight='bold', ha='center', va='bottom', transform=ccrs.PlateCarree(), zorder=15, bbox=dict(boxstyle='round,pad=0.2', facecolor='#1a5490', edgecolor='white', linewidth=1.3, alpha=1), path_effects=[PathEffects.Normal()])
        ax.text(lon_ci, lat_ci - 0.14, nombre, fontsize=9, color='#2C3E50', weight='bold', ha='center', va='top', zorder=15, transform=ccrs.PlateCarree(), path_effects=[PathEffects.withStroke(linewidth=2, foreground='white'), PathEffects.SimpleLineShadow(offset=(1, -1), alpha=0.3), PathEffects.Normal()])
    if show_logo:
        try:
            logo_img = mpimg.imread(LOGO_PATH)
    ...
    output_file = f'output/maps/texas/texas_map_wind.png'
    plt.savefig(output_file, dpi=800, bbox_inches='tight')
    print(f'✅ Map saved: {output_file}')
    plt.show()
    return (fig, ax)
...
def plot_florida_wind(ds_wind, admin_boundaries=None, florida_counties=None, show_logo=True):
    """
    Plot wind magnitude with direction vectors para Florida en MPH
    ...
    lat_min = 24.3
    lat_max = 31.5
    if 'u10' not in ds_wind or 'v10' not in ds_wind or 'wind_speed' not in ds_wind:
        raise ValueError("Dataset debe contener 'u10', 'v10' y 'wind_speed'")
    lon_orig_u = ds_wind['u10'].longitude.values
    lon_corrected_u = np.where(lon_orig_u > 180, lon_orig_u - 360, lon_orig_u)
    u10_corrected = ds_wind['u10'].assign_coords(longitude=lon_corrected_u)
    v10_corrected = ds_wind['v10'].assign_coords(longitude=lon_corrected_u)
    ...
    print(f'  LON: [{lon_orig_u.min():.2f}, {lon_orig_u.max():.2f}] - {len(lon_orig_u)} points')
    u10_area = u10_corrected.sel(latitude=slice(lat_max + 1, lat_min - 1), longitude=slice(lon_min_deg, lon_max_deg))
    v10_area = v10_corrected.sel(latitude=slice(lat_max + 1, lat_min - 1), longitude=slice(lon_min_deg, lon_max_deg))
    wind_area = wind_speed_corrected.sel(latitude=slice(lat_max + 1, lat_min - 1), longitude=slice(lon_min_deg, lon_max_deg))
    lat = wind_area.latitude.values
    lon = wind_area.longitude.values
    print(f'\nSelected area:')
    ...
    cbar.ax.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'{int(x)}'))
    cbar.ax.tick_params(labelsize=8)
    major_cities = {'Miami': {'lat': 25.76, 'lon': -80.19}, 'Tampa': {'lat': 27.95, 'lon': -82.46}, 'Orlando': {'lat': 28.54, 'lon': -81.38}, 'Jacksonville': {'lat': 30.33, 'lon': -81.66}, 'Tallahassee': {'lat': 30.44, 'lon': -84.28}, 'West Palm Beach': {'lat': 26.71, 'lon': -80.05}, 'Naples': {'lat': 26.14, 'lon': -81.79}, 'Fort Myers': {'lat': 26.64, 'lon': -81.87}, 'Sarasota': {'lat': 27.34, 'lon': -82.53}, 'Pensacola': {'lat': 30.42, 'lon': -87.22}, 'Gainesville': {'lat': 29.65, 'lon': -82.32}, 'Daytona Beach': {'lat': 29.21, 'lon': -81.02}, 'Key West': {'lat': 24.56, 'lon': -81.78}, 'Panama City': {'lat': 30.16, 'lon': -85.66}}
    flat_points = np.column_stack((lat2d_fine.ravel(), lon2d_fine.ravel()))
    tree = KDTree(flat_points)
    wind_values = wind_fine_display.ravel()
    for nombre, datos in major_cities.items():
    ...
    output_file = f'{output_dir}/florida/florida_map_wind.png'
    plt.savefig(output_file, dpi=800, bbox_inches='tight')
    print(f'✅ Map saved: {output_file}')
    plt.show()
    return (fig, ax)
...
def plot_mexico_wind(ds_wind, admin_boundaries=None):
    """
    Plot wind magnitude with direction vectors for Mexico
    ...
    ax = plt.axes(projection=ccrs.PlateCarree())
    buffer_lat = 1
    ax.set_extent([lon_min_display, lon_max_display + buffer_lat, lat_min_display - buffer_lat, lat_max_display + buffer_lat], crs=ccrs.PlateCarree())
    ax.add_feature(cfeature.LAND, facecolor='#E8E8E8', alpha=0.3)
    ax.add_feature(cfeature.OCEAN, facecolor='lightblue', alpha=0.3)
    lakes_feature = cfeature.NaturalEarthFeature('physical', 'lakes', '10m', edgecolor='black', facecolor='lightblue')
    ax.add_feature(lakes_feature, linewidth=0.5, alpha=0.3, zorder=1)
    wind_colors = ['#e0f7fa', '#80deea', '#26c6da', '#00acc1', '#00838f', '#4db6ac', '#ffca28', '#ff7043', '#f44336', '#8e24aa']
    cmap_wind = ListedColormap(wind_colors)
    cmap_truncated = cmap_wind
    vmin = np.nanmin(wind_fine_display)
    ...
    level_count = 30
    levels = np.linspace(vmin, vmax, level_count)
    cf = ax.contourf(lon2d_fine, lat2d_fine, wind_fine_display, levels=levels, cmap=cmap_truncated, extend='both', transform=ccrs.PlateCarree(), alpha=0.7, zorder=2)
    step = 4
    wind_magnitude = np.sqrt(u_vals ** 2 + v_vals ** 2) * 3.6
    mask_lon = (lon2d >= lon_min_display) & (lon2d <= lon_max_display)
    mask_lat = (lat2d >= lat_min_display) & (lat2d <= lat_max_display)
    mask_display = mask_lon & mask_lat
    u_sub = u_vals[::step, ::step]
    v_sub = v_vals[::step, ::step]
    wind_mag_sub = wind_magnitude[::step, ::step]
    lon_sub = lon2d[::step, ::step]
    lat_sub = lat2d[::step, ::step]
    mask_sub = mask_display[::step, ::step]
    u_plot = np.where(mask_sub, u_sub, np.nan)
    v_plot = np.where(mask_sub, v_sub, np.nan)
    wind_scale = np.log1p(wind_mag_sub)
    wind_scale = wind_scale / np.nanmax(wind_scale)
    u_scaled = u_plot * (0.3 + 0.7 * wind_scale)
    v_scaled = v_plot * (0.3 + 0.7 * wind_scale)
    quiver = ax.quiver(lon_sub, lat_sub, u_scaled, v_scaled, transform=ccrs.PlateCarree(), color='black', alpha=0.75, width=0.0025, scale=80, scale_units='inches', headwidth=3.5, headlength=4, headaxislength=3.5, minshaft=2, minlength=0.1, zorder=10)
    ...
    cbar = plt.colorbar(cf, cax=cax, orientation='horizontal')
    cbar.set_label('Rachas maximum de wind (km/h)', fontsize=11, weight='bold')
    cbar.ax.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'{int(x)}'))
    cbar.ax.tick_params(labelsize=8)
    ...
    for nombre, datos in major_cities.items():
        ...
        _, idx = tree.query([lat_ci, lon_ci])
        wind_speed = wind_values[idx]
        ax.plot(lon_ci, lat_ci, 'o', color='red', markersize=4, markeredgecolor='white', markeredgewidth=1, transform=ccrs.PlateCarree(), zorder=20)
        ax.text(lon_ci, lat_ci + 0.18, f'{wind_speed:.0f} ', fontsize=11.5, color='white', weight='bold', ha='center', va='bottom', transform=ccrs.PlateCarree(), zorder=21, bbox=dict(boxstyle='round,pad=0.195', facecolor='#1a5490', edgecolor='white', linewidth=1.3, alpha=1), path_effects=[PathEffects.Normal()])
        ax.text(lon_ci, lat_ci - 0.12, nombre, fontsize=8, color='#2C3E50', weight='bold', ha='center', va='top', zorder=21, transform=ccrs.PlateCarree(), path_effects=[PathEffects.withStroke(linewidth=2, foreground='white'), PathEffects.SimpleLineShadow(offset=(1, -1), alpha=0.3), PathEffects.Normal()])
    ...
    output_file = f'output/maps/mexico/mexico_map_wind.png'
    plt.savefig(output_file, dpi=800, bbox_inches='tight')
    print(f'Map saved: {output_file}')
    plt.show()
    return (fig, ax)
...
def plot_cuba_wind(ds_wind, cuba=None, admin_boundaries=None):
    """
    Plot wind magnitude with direction vectors for Cuba
    ...
    cmap_wind = ListedColormap(wind_colors)
    cmap_truncated = cmap_wind
    vmin = np.nanmin(wind_masked_fine)
    vmax = np.nanmax(wind_masked_fine)
    level_count = 15
    levels = np.linspace(vmin, vmax, level_count)
    cf = ax.contourf(lon2d_fine, lat2d_fine, wind_masked_fine, levels=levels, cmap=cmap_truncated, extend='both', transform=ccrs.PlateCarree(), alpha=0.8)
    step = 2
    wind_magnitude = np.sqrt(u_vals ** 2 + v_vals ** 2) * 3.6
    u_sub = u_vals[::step, ::step]
    v_sub = v_vals[::step, ::step]
    ...
    flat_points = np.column_stack((lat2d_fine.ravel(), lon2d_fine.ravel()))
    tree = KDTree(flat_points)
    wind_values = wind_masked_fine.ravel()
    for nombre, datos in major_cities.items():
        lat_ci = datos['lat']
        lon_ci = datos['lon']
        _, idx = tree.query([lat_ci, lon_ci])
        wind_speed = wind_values[idx]
        ax.plot(lon_ci, lat_ci, 'o', color='red', markersize=4, markeredgecolor='white', markeredgewidth=1, transform=ccrs.PlateCarree(), zorder=12)
        ax.text(lon_ci, lat_ci + 0.08, f'{wind_speed:.0f} ', fontsize=11.5, color='white', weight='bold', ha='center', va='bottom', transform=ccrs.PlateCarree(), zorder=15, bbox=dict(boxstyle='round,pad=0.2', facecolor='#1a5490', edgecolor='white', linewidth=1.3, alpha=1), path_effects=[PathEffects.Normal()])
    ...
    plt.savefig(f'output/maps/cuba/cuba_map_wind.png', dpi=800, bbox_inches='tight')
    plt.show()
    return (fig, ax)
